...
class UnifiedAlarmResolverTest {
    ...
    @Test fun everyDayFiresLaterToday() {
        val a = UnifiedAlarm(id = "a", wakeMinutes = 18 * 60)   // 18:00 every day
        val now = epoch(2026, 6, 23, 10, 0)
        assertEquals(epoch(2026, 6, 23, 18, 0), UnifiedAlarmResolver.nextFireAtEpochMs(a, now, utc))
    }

    @Test fun everyDayFiresTomorrowWhenTodayIsPast() {
        val a = UnifiedAlarm(id = "a", wakeMinutes = 6 * 60)    // 06:00 every day
        val now = epoch(2026, 6, 23, 10, 0)
