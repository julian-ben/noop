 ...
 *
 * Persisted as JSON via kotlinx-serialization. iOS counterpart (Strand/Data/UnifiedAlarm.swift) is
 * a follow-up: this build ignores unknown keys (`ignoreUnknownKeys = true`) so older payloads still
 * decode, but the Swift side will need a parallel migration before cross-platform sync ships.
 */
@Serializable
