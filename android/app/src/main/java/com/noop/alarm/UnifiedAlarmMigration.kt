 ...
 *
 * Mirrors the iOS SmartAlarmMigration so a fresh install on either platform reaches the same shape.
 */
object UnifiedAlarmMigration {
    ...
    fun migrate(
        ...
        if (newPrefs.getBoolean(UnifiedAlarmStore.KEY_MIGRATED, false)) return
        ...
        if (!strapOn && !phoneOn) {
            newPrefs.edit().putBoolean(UnifiedAlarmStore.KEY_MIGRATED, true).apply()
            return
        }
        ...
        val baseWeekdays = if (baseSource == AlarmSource.PHONE) {
            emptySet()  // phone-only legacy alarm didn't have weekdays
        } else {
            strapWeekdays - overrides.keys
        }
